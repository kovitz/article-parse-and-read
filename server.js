...
app.post('/api/parse', async (req, res) => {
  try {
    ...
    if (!article) {
      return res.status(400).json({ error: 'Could not extract article content from this URL' });
    }
    ...
    res.status(500).json({ error: 'An error occurred while parsing the article: ' + error.message });
  }
});
